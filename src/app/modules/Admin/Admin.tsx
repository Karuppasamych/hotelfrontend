...
import { adminApi, AdminSettings } from '../utils/adminApi';
import { Settings, Save, Percent, RefreshCw, Shield, LayoutDashboard, Package, ChefHat, Calculator, Receipt, ShoppingCart, History, UtensilsCrossed, Users } from 'lucide-react';
...
export default function Admin() {
  ...
  const handleSaveAccess = async () => {
    ...
    try {
      ...
      if ((response as any).success !== false) {
        toast.success('Menu access saved! Users will see changes on next login.');
      } else {
        toast.error('Failed to save access settings');
      }
    } catch {
      toast.error('Error saving access settings');
    }
    setSavingAccess(false);
  };

  const toggleMenuAccess = (role: string, menu: string) => {
    // Don't allow removing admin's access to users/admin pages
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="flex gap-2 mb-6">
          <button
            ...
            className={`px-5 py-2.5 rounded-xl font-bold text-sm transition-all ${activeTab === 'billing' ? 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-md' : 'bg-white text-gray-600 border-2 border-gray-200 hover:border-gray-300'}`}
          >
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-xl font-bold text-sm transition-all ${activeTab === 'access' ? 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white shadow-md' : 'bg-white text-gray-600 border-2 border-gray-200 hover:border-gray-300'}`}
          >
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="p-5 bg-gradient-to-r from-orange-50 to-amber-50 rounded-xl border-2 border-orange-200">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    onClick={() => setSettings(prev => ({ ...prev, service_charge_enabled: prev.service_charge_enabled === 'true' ? 'false' : 'true' }))}
              ...
              <div className="p-5 bg-gray-50 rounded-xl border-2 border-gray-200">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {serviceChargeEnabled && <div className="flex justify-between text-gray-600"><span>Service Charge ({settings.service_charge_percent}%)</span><span className="font-medium">₹{(1000 * parseFloat(settings.service_charge_percent || '0') / 100).toFixed(2)}</span></div>}
                  <div className="flex justify-between text-gray-600"><span>CGST ({settings.cgst_percent}%)</span><span className="font-medium">₹{(1000 * parseFloat(settings.cgst_percent || '0') / 100).toFixed(2)}</span></div>
                  <div className="flex justify-between text-gray-600"><span>SGST ({settings.sgst_percent}%)</span><span className="font-medium">₹{(1000 * parseFloat(settings.sgst_percent || '0') / 100).toFixed(2)}</span></div>
                  <div className="flex justify-between text-gray-900 pt-2 border-t-2 border-gray-300 font-bold">
                    <span>Total</span>
                    <span>₹{(1000 + (serviceChargeEnabled ? 1000 * parseFloat(settings.service_charge_percent || '0') / 100 : 0) + 1000 * parseFloat(settings.cgst_percent || '0') / 100 + 1000 * parseFloat(settings.sgst_percent || '0') / 100).toFixed(2)}</span>
                  </div>
                </div>
              </div>
            </div>
                    ...
                    <tr>
                      ...
                      {ROLES.map(role => (
                        <th key={role.value} className="text-center pb-3 px-2">
                          <span className={`inline-block px-3 py-1 bg-gradient-to-r ${role.color} text-white text-xs font-bold rounded-full`}>
                            {role.label}
                          </span>
                        </th>
                      ))}
                          ...
                          {ROLES.map(role => {
                            ...
                            return (
                              <td key={role.value} className="text-center py-3 px-2">
                                <button
                                  onClick={() => toggleMenuAccess(role.value, menu.key)}
                                  disabled={isLocked}
                                  className={`w-6 h-6 rounded-md border-2 flex items-center justify-center transition-all mx-auto ${
                                    isLocked
                                      ? 'border-green-500 bg-green-500 cursor-not-allowed opacity-70'
                                      : hasAccess
                                        ? 'border-green-500 bg-green-500 hover:bg-green-600 cursor-pointer'
